...
public class AzureOpenAIProvider extends LLMProvider {
    ...
    public AzureOpenAIProvider(LLMProviderConfig config, String apiKey, String endpoint) {
        ...
        if (endpoint == null) {
            ...
            this.client = new OpenAIClientBuilder().credential(new KeyCredential(apiKey)).serviceVersion(OpenAIServiceVersion.V2025_01_01_PREVIEW).buildClient();
        } else {
            this.client = new OpenAIClientBuilder().credential(new AzureKeyCredential(apiKey)).endpoint(endpoint).serviceVersion(OpenAIServiceVersion.V2025_01_01_PREVIEW).buildClient();
        }
    }
