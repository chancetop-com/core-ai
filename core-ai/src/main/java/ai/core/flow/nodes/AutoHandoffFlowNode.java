...
public class AutoHandoffFlowNode extends HandoffFlowNode<AutoHandoffFlowNode> {
    ...
    public void init(List<FlowNode<?>> settings, List<FlowEdge<?>> edges) {
        var agentNode = settings.stream().filter(v -> v instanceof AgentFlowNode).findFirst().orElseThrow(() -> new IllegalArgumentException("AutoHandoffFlowNode must have AgentFlowNode as setting: " + this.getName()));
    ...
    public void check(List<FlowNode<?>> settings) {
        ...
        if (!agentExist) {
            throw new IllegalArgumentException("AutoHandoffFlowNode must have AgentFlowNode as setting: " + this.getName());
        }
    }
