 ...
 * author: lim chen
 * date: 2026/03/16
 * description: copy from {@link core.framework.internal.http.HTTPClientImpl} to solve version compatibility issues
 */
public final class CustomHTTPClientImpl implements HTTPClient {
    ...
    private final Logger logger = LoggerFactory.getLogger(CustomHTTPClientImpl.class);
    private final String userAgent;
    private final OkHttpClient client;

    public CustomHTTPClientImpl(OkHttpClient client, String userAgent, Duration timeout) {
        this.client = client;
    ...
    Request httpRequest(HTTPRequest request) {
        ...
        if (!request.params.isEmpty())
            logger.debug("[request] params={}", new FieldMapLogParam(request.params));
    ...
    MediaType mediaType(@Nullable ContentType contentType) {
        ...
        if (ContentType.APPLICATION_JSON.equals(contentType)) return MEDIA_TYPE_APPLICATION_JSON;
        return MediaType.get(contentType.toString());
    }
}
