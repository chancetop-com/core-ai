...
public class GatewayProviderService {
    ...
    private TestGatewayProviderResponse test(GatewayProviderConfig entity) {
        ...
        try {
            ...
            } else if (isMediaTaskProvider(entity)) {
                GatewayMediaProbe.test(entity, secret(entity), result);
            } else {
    ...
    /** Protocols whose credential is verified by a probe against their own async task API, not by /models. */
    private boolean isMediaTaskProvider(GatewayProviderConfig provider) {
        return "KIE".equals(provider.mediaProtocol) || "VOLCENGINE_ARK".equals(provider.mediaProtocol);
    }
    ...
    private String normalizeMediaProtocol(String mediaProtocol) {
        ...
        return switch (value) {
            case "OPENAI_IMAGES", "OPENAI_COMPATIBLE", "GEMINI_GENERATE_CONTENT", "VERTEX_GEMINI_GENERATE_CONTENT", "VERTEX_GEMINI_INTERACTIONS", "KIE", "VOLCENGINE_ARK" -> value;
