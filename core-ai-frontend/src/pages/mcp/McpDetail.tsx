...
import { ArrowLeft, Server, RefreshCw, Plug, Play, ChevronDown, ChevronRight, Wrench, Loader2, Zap, Eye, EyeOff } from 'lucide-react';
...
import { containsSensitiveConfigJson, isSensitiveConfigKey } from './mcpConfig';
...
export default function McpDetail() {
  ...
  const [rawConfigRevealed, setRawConfigRevealed] = useState(false);
  ...
  const rawConfigContainsSecrets = rawConfigDisplay ? containsSensitiveConfigJson(rawConfigDisplay) : false;
      ...
      <div className="rounded-xl border p-4 mb-4"
        ...
        {(Object.keys(server.config || {}).length > 0 || rawConfigDisplay) && (
          <div className="mt-4 pt-3 border-t" style={{ borderColor: 'var(--color-border)' }}>
            ...
            {server.config?.transport === 'sandbox_hosted' ? (
              <div>
                {rawConfigContainsSecrets && !rawConfigRevealed ? (
                  <div className="text-xs font-mono p-2 rounded"
                    style={{ background: 'var(--color-bg-tertiary)', color: 'var(--color-text-secondary)' }}>
                    ••••••••••••••••
                  </div>
                ) : (
                  <pre className="text-xs font-mono p-2 rounded overflow-auto max-h-60 whitespace-pre-wrap break-all"
                    style={{ background: 'var(--color-bg-tertiary)', color: 'var(--color-text-secondary)' }}>
                    {rawConfigDisplay || 'No raw config'}
                  </pre>
                )}
                {rawConfigContainsSecrets && (
                  <button type="button" onClick={() => setRawConfigRevealed(value => !value)}
                    aria-label={`${rawConfigRevealed ? 'Hide' : 'Reveal'} raw configuration`}
                    className="mt-2 flex items-center gap-1 text-xs cursor-pointer"
                    style={{ color: 'var(--color-primary)' }}>
                    {rawConfigRevealed ? <EyeOff size={13} /> : <Eye size={13} />}
                    {rawConfigRevealed ? 'Hide sensitive configuration' : 'Reveal sensitive configuration'}
                  </button>
                )}
              </div>
            ) : (
              <div className="grid gap-2 grid-cols-1 sm:grid-cols-2">
                {Object.entries(server.config).map(([k, v]) => {
                  ...
                  return (
                    <div key={k} className="flex items-start gap-1 text-xs font-mono rounded px-2 py-1 min-w-0 overflow-hidden"
                      style={{ background: 'var(--color-bg-tertiary)' }}>
                      <span className="shrink-0" style={{ color: 'var(--color-text-secondary)' }}>{k}: </span>
                      <span className="min-w-0 flex-1 break-all">{sensitive && !revealed ? '••••••••••••••••' : v}</span>
                      {sensitive && (
