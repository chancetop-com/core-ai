...
import { X, FileText, Code as CodeIcon, FileCode, Globe, Image as ImageIcon, Download, Copy, Check } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full w-[520px] shrink-0 border-l"
      ...
      <div className="flex-1 min-h-0 overflow-auto" style={{ background: 'var(--color-bg)' }}>
        {mode === 'preview' && renderPreview(artifact)}
...
function renderPreview(spec: ArtifactSpec) {
  ...
  if (spec.kind === 'file' && spec.fileId) {
    const url = `/api/files/${spec.fileId}/content`;
    if (spec.contentType?.startsWith('image/')) {
      return <div className="p-6 flex items-center justify-center"><img src={url} alt={spec.title} className="max-w-full max-h-full" /></div>;
    }
    if (spec.contentType === 'text/html') {
      return <iframe sandbox="allow-scripts" src={url} title={spec.title} className="w-full h-full border-0" />;
    }
