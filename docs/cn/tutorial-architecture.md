...
4. [工具执行机制](#工具执行机制)
5. [消息处理流程](#消息处理流程)
6. [AgentGroup 协作机制](#agentgroup-协作机制)
7. [Flow 执行引擎](#flow-执行引擎)
8. [追踪与遥测](#追踪与遥测)
...
│     设置系统变量: agents信息、maxRound、terminationWord      │
│                                                             │
│  2. startRunning()                                          │
│     状态 → RUNNING, round = 1                               │
│                                                             │
...
public record AutoHandoff(Agent moderator) implements Handoff {
    ...
    public void handoff(AgentGroup agentGroup, Planning planning,
        ...
        // 让 moderator Agent 进行规划决策
        // moderator 的 systemPrompt 包含了所有可用 Agent 的信息
        // 它会输出 JSON 格式的规划结果
...
public interface Planning {
    /**
     * Agent 规划模式：让 moderator Agent 思考下一步行动
     * @return moderator 的原始输出（JSON 格式）
     */
    String agentPlanning(Agent agent, String query, Map<String, Object> variables);

    /**
     * 直接设置规划结果（用于 DirectHandoff）
     */
     ...
     * 获取规划结果
     */
    String nextAgentName();  // 下一个执行的 Agent 名称
    String nextQuery();      // 传递给下一个 Agent 的查询
    String nextAction();     // 下一步动作（可能是终止词）
}
```
...
│    ├─ FlowNodeType.AGENT_GROUP - AgentGroup 节点            │
...
5. **多 Agent 协作**：Handoff + Planning 双重机制，灵活的协作模式
6. **Flow 编排**：有向图执行引擎，支持条件路由和事件监听
7. **可观测性**：原生 OpenTelemetry 支持，兼容主流观测平台
