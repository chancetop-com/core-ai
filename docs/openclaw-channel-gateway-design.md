...
OCG 发送标准 OpenAI chat completion 请求，回调地址放在 HTTP Header 中，请求体不含任何自定义字段：
...
`callbackSecret` 由用户配置在 OCG 的 `ocg.json` 和 core-ai OCG Config 中（双方共享）。
...
- **OCG Config**：通过 `channelId` 单向关联 Channel。用户上传原始 JSON 配置，其中必须包含 `async: true`
- **Sandbox**：持久运行，timeout 设为 86400s（24h），由 `OcgHealthCheckJob` 定时 renew

> 关系说明：一个 OCG 实例对应一个 Channel 入口和一个 Sandbox。OCG 内部可配置多个 IM 渠道，所有渠道的消息都转发到同一个 core-ai Channel 入口。如果需要不同 Agent 处理不同渠道，需创建多个 Channel + 多个 OCG Config。

## 6. 用户操作流程

```
1. 创建 Channel (type="openclaw") → 系统生成 webhookUrl
2. 创建 OCG Config → 选择关联的 Channel → agentUrl 自动填入 webhookUrl → 上传原始 JSON 配置
3. 点击"启动 Sandbox" → 系统创建 sandbox → 获取 sandbox IP
   → 注入 ocg.json（async: true, callbackHost: <sandboxIP>）
   → 执行 ocg start
...
**configJson 字段说明**：参考 `ToolRegistry.rawConfig` 的模式，原始 JSON 以字符串形式存储，方便用户查看和编辑。OCG 异步模式要求在 configJson 中包含：
...
其中 `agentUrl`、`apiKey`、`callbackHost` 由系统在启动时自动注入/覆盖。
...
异步分支流程：
1. 验证 `Authorization: Bearer` header：提取 token，与系统配置的 API key（`sys.ocg.api-key`）比对。不匹配返回 401。OCG 侧 `ocg.json` 中的 `apiKey` 字段由系统注入与此一致的 key
2. 解析 OpenAI body（复用已有解析逻辑）
3. 读取 `X-OCG-Callback` header 获取回调 URL
4. 查找 Channel → Agent
5. 创建/复用 Agent session，派发消息
...
```json
{
  ...
  "configJson": "{ \"async\": true, \"channels\": { \"telegram\": { ... } } }",
  "callbackSecret": "可选"
}
```
...
1. `configJson` 必须能解析为合法 JSON
2. 解析后必须包含 `"async": true`（OCG 异步模式为硬性要求），否则返回 400
3. `channelId` 对应的 Channel 必须存在且类型为 `"openclaw"`
...
2. 对每个记录，调用 `SandboxProvider.attach(sandboxId)` 尝试重连已有 pod：
...
void recoverOnStartup() {
    for (var config : ocgConfigStore.allWithSandbox()) {
        var sandbox = sandboxManager.provider.attach(
            config.sandboxId, buildSandboxConfig(config), "ocg-" + config.id, "system"
        );
        if (sandbox.isPresent()) {
            sandboxManager.register(sandbox.get());  // 重新注册到内存
...
> 注意：`SandboxManager` 需要新增 `register(Sandbox)` 方法（绕过 `acquire`，直接注册已有 sandbox 到 `activeSandboxes`），以及暴露 `provider` 字段供外部调用 `attach`。
...
public class OcgCallbackPool {
    ...
    @Inject static ChatMessageService chatMessageService;
    @Inject static OcgConfigStore ocgConfigStore;

    public static void submit(String sessionId, String callbackUrl, String channelId) {
        executor.submit(() -> {
            var reply = pollForResponse(sessionId);
            var callbackSecret = ocgConfigStore.loadByChannelId(channelId).callbackSecret;
            if (reply != null) {
    ...
    private static void postCallback(String url, String reply, boolean isError, String secret) {
        var body = Map.of("reply", reply, "isError", isError);
        var bodyJson = JsonUtil.toJson(body);
        var headers = new HashMap<String, String>();
        headers.put("Content-Type", "application/json");
        if (secret != null) {
            headers.put("X-OCG-Signature", "sha256=" + hmacSha256(bodyJson, secret));
        }
...
> 不需要注册新路由，`OcgCallbackPool` 通过 `@Inject` 注入到 `ChannelSyncController` 即可。
>
> **Session 生命周期**：异步模式下 `createSessionFromAgent` 创建的 ChatSession 不会被显式关闭。`IdleSessionCleanupJob`（已有，每 5 分钟）会自动清理超时 session，无需额外处理。若需要即时清理，可在 `OcgCallbackPool.pollForResponse()` 获取回复后调用 `sessionManager.closeSession(sessionId)`。
...
2. 对每个运行中的 OCG：
   ...
   - 检查 OCG 进程是否正常运行（在 sandbox 内执行 `ocg status`）
...
- **新建/编辑弹窗（右侧抽屉）**：
  ...
  - agentUrl（自动填入选中 Channel 的 webhookUrl，只读展示）
  - JSON 配置（大文本框，用户直接粘贴完整 JSON）

### 14.3 Channel 创建时的联动

已有 Channels 创建页面中，当用户选择 Channel Type 为 "OpenClaw" 时，无需额外字段。Channel 创建完拿到 webhookUrl 后，再去 OpenClaw 页面创建 OCG Config（系统自动将 agentUrl 注入到 configJson 中）。
...
1. **OCG JSON 配置表单化**：当前为原始 JSON 文本框，后续可以提供结构化表单（Telegram Bot Token、Discord Token 等分项填写）
2. **OCG 同步模式支持**：如需短平快的同步对话，可支持 OCG 同步模式 → `ChannelSyncController`（已有）
3. **OCG 多实例**：当前 Channel 与 OCG Config 为 1:1，后续可考虑一个 OCG 对接多个 Channel 入口（不同 Agent）
4. **OCG 日志查看**：sandbox 内的 OCG 日志可回传到 core-ai-server 展示
5. **OCG 插件管理**：前端界面支持安装/卸载 OCG 渠道插件
6. **Sandbox 自动重启**：health check 检测到异常时可自动尝试重启
7. **Sandbox 网络策略**：K8s NetworkPolicy 限制 OCG sandbox 只能访问 IM 平台 API 域名 + core-ai-server webhook URL
